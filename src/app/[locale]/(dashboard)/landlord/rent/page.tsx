...
export default async function LandlordRent({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Track rent payments from your tenants</p>
      </div>
      ...
      {rentSchedules.length === 0 ? (
        <div className="rounded-xl border bg-white p-12 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Rent tracking will appear here once you have active agreements with tenants
          </p>
        </div>
      ) : (
              ...
              <div key={schedule.id} className="flex items-center gap-4 p-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {schedule.agreement.tenant.fullName} &middot; Due {new Date(schedule.dueDate).toLocaleDateString('en-NG', { day: 'numeric', month: 'short', year: 'numeric' })}
                  </p>
                </div>
